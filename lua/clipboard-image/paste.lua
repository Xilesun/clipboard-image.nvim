...
local paste_img_to = function(path, img)
  os.execute(string.format(cmd_paste, path, img))
end
...
M.paste_img = function(opts)
  ...
  local path = utils.resolve_dir(conf.img_dir)
  ...
  if type(conf.img_handler) == "function" then
    conf.img_handler { name = conf.img_name, path = path }
  end
  -- end
end
